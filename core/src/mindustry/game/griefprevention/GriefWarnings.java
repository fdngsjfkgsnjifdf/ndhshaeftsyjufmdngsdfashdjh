...
public class GriefWarnings {
    ...
    public void handleBlockConstructProgress(Player builder, Tile tile, Block cblock, float progress, Block previous) {
        ...
        if (coreDistance < 30 && cblock instanceof NuclearReactor) {
            String message = "[royal]ALERT![] " + formatPlayer(builder) + " is building a reactor [stat]" +
                    Math.round(coreDistance) + "[] blocks from core. [stat]" + Math.round(progress * 100) + "%" + formatTile(tile);
            sendMessage(message);
            didWarn = true;
        }
        ...
        if (!info.constructSeen) {
            ...
            if (!didWarn) {
                if (cblock instanceof NuclearReactor) {
                    ...
                    if (!hasCryo) {
                        sendLocal("reactor being built at" + formatTile(tile));
                    }
                    }
                }
            }
    ...
    public void handlePowerGraphSplit(Player targetPlayer, Tile tile, PowerGraph oldGraph, PowerGraph newGraph1, PowerGraph newGraph2) {
        ...
        if (Math.min(oldGraphCount - newGraph1Count, oldGraphCount - newGraph2Count) > 20) {
            sendMessage("[yellow]ALERT![] Power split by " + formatPlayer(targetPlayer) + "\n " + oldGraphCount + " -> " +
    ...
    public void handleBlockBeforeConfigure(Tile tile, Player targetPlayer, int value) {
        ...
        if (targetPlayer != null) {
            ...
            if (tile.block() != Blocks.itemSource && stats.configureRatelimit.get()) {
                stats.configureRatelimit.nextTick(rl -> sendMessage("[red]ALERT![] Configure ratelimit " + formatRatelimit(rl, targetPlayer)));
            }
        }
    ...
    public void handleRotateBlock(Player targetPlayer, Tile tile, boolean direction) {
        ...
        if (targetPlayer != null) {
            ...
            if (stats.rotateRatelimit.get()) {
                stats.rotateRatelimit.nextTick(rl -> sendMessage("[pink]ALERT![] Rotate ratelimit " + formatRatelimit(rl, targetPlayer)));
            }
        }
    ...
    public void handleThoriumReactorHeat(Tile tile, float heat) {
        if (heat > 0.15f && tile.interactable(player.getTeam())) {
            sendMessage("[royal]ALERT![] Thorium reactor at " + formatTile(tile) + " is overheating! Heat: [accent]" + heat);
        }
    }
