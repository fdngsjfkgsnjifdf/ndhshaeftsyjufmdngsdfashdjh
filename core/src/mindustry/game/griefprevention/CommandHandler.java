...
public class CommandHandler {
    ...
    public CommandHandler() {
        ...
        addCommand("", settingsToggle("verbose", "verbose logging", v -> griefWarnings.verbose = v));
        addCommand("", settingsToggle("debug", "debug logging", v -> griefWarnings.debug = v));
        addCommand("", settingsToggle("spam", "verbose and debug logging", v -> {
            griefWarnings.verbose = v;
            griefWarnings.debug = v;
        }));
        addCommand("cast", settingsToggle("broadcast", "broadcast of messages", v -> griefWarnings.broadcast = v));
        ...
        addCommand("vk", this::votekick);
        addCommand("tileinfohud", settingsToggle("tileinfohud", "tile information hud", v -> griefWarnings.tileInfoHud = v));
        addCommand("", settingsToggle("autoban", "automatic bans", v -> griefWarnings.autoban = v));
        addCommand("", settingsToggle("autotrace", "automatic trace", v -> griefWarnings.autotrace = v));
        addCommand("auto", this::auto);
        addCommand("a", this::auto);
        addCommand("nextwave", this::nextwave);
        addCommand("nw", this::nextwave);
        addCommand("playerinfo", this::playerInfo);
        addCommand("pi", this::pi); // playerinfo takes too long to type
        addCommand("", this::eval);
        addCommand("freecam", createToggle("freecam", "free movement of camera", v -> griefWarnings.auto.setFreecam(v)));
        addCommand("cam", createToggle("freecam", "free movement of camera", v -> griefWarnings.auto.setFreecam(v)));
    ...
    public void fixPower(CommandContext ctx) {
        boolean redundant = ctx.args.contains("");
    ...
    public void playerInfo(CommandContext ctx) {
      ...
      /** Get information about a player */
      public void pi(CommandContext ctx) {
        String name = String.join(" ", ctx.args.subList(1, ctx.args.size()));
        PlayerStats stats = getStats(name);
        if (stats == null) {
            reply("[scarlet]Not found");
        ...
        reply(pi);
        StringSelection stringSelection = new StringSelection(pi);
    ...
    public void auto(CommandContext ctx) {
        ...
        switch (ctx.args.get(1).toLowerCase()) {
            ...
            case "gotop": {
                if (ctx.args.size() < 3) {
                    reply("[scarlet]Not enough arguments");
                    reply("Usage: auto gotop [follow|assist|undo] <player>");
                    return;
                }
                int nameStart = 2;
